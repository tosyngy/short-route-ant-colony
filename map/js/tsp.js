...
function formatDirections(gdir, mode) {
    ...
    for (var i = 0; i < gdir.legs.length; ++i) {
        ...
        + "<div class='centered-directions'><img src='http://gebweb.net/optimap/newicons/black"
        + number + ".png'></div></td>"
        ...
        + (i ? "default" : "disabled") + "'>"
        + "<table class='dragTable'><tr><td class='left'><img src='http://gebweb.net/optimap/newicons/black"
    ...
    if (mode == 0) {
        ...
        + "<table class='dragTable'><tr><td><img src='http://gebweb.net/optimap/newicons/black"
        + 1 + ".png' /></td><td>" + headerStr
        + "</td></tr></table></li>"; 
        retStr += "\t<tr class='heading'><td class='heading'>"
        + "<div class='centered-directions'><img src='http://gebweb.net/optimap/newicons/black1.png'></div></td>"
        + "<td class='heading'>"
...
function drawMarker(latlng, addr, label, num) {
    var icon;
    icon = new google.maps.MarkerImage("http://gebweb.net/optimap/newicons/red" + (num + 1) + ".png");
...
function clickedAddAddress() {
    addr+=document.address.addressStr.value + "+";
...
function onSolveCallback(myTsp) {
   ...
   // document.getElementById("path").innerHTML = pathStr;
     ...
     //alert(addr);
    jQuery.get("http://antcolony.xisysgroup.com/map/getconstraits.php",function(data){
        var report=JSON.parse(data);
        jQuery.each(report,function(id,value){
            // console.log(id);
            if(addr.indexOf(value.to_location)>-1 && addr.indexOf(value.from_location)>-1){
                str+="<div style='margin:3px'><h3>Road Constraits Info from<b> "+value.from_location+" to "+value.to_location+"</b></h3>";
                str+="<p>Highway Blocked: "+value.highway_block+"</p>";
                str+="<p>Vechile Rate/sec: "+value.vehicle_rate+"</p>";
                str+="<p>Cross Threshold: "+value.cross_rate+"</p>"+pathStr;
                str+="<p><b>Delay Time(mins): "+value.delay+"</b></p></div>";
                str+="<p><b>Total Estimated Time: "+formatTime((parseInt(value.delay)*60)+parseInt(getTotalDuration(dir)))+"</b></p></div>";
                 // console.log(str);
            }
        });
         jQuery("#road_info").html(str);
    })
    ...
    if (mode == 1) {
        ...
        var myIcn1 = new google.maps.MarkerImage("http://gebweb.net/optimap/newicons/black1.png");
        var marker = new google.maps.Marker({ 
            position: myPt1, 
            icon: myIcn1, 
            map: gebMap
    ...
    for (var i = 0; i < dir.legs.length; ++i) {
        ...
        if (i == dir.legs.length - 1 && mode == 0) {
            myIcn1 = new google.maps.MarkerImage("http://gebweb.net/optimap/newicons/black1.png");
        } else {
            myIcn1 = new google.maps.MarkerImage("http://gebweb.net/optimap/newicons/black" + (i+2) + ".png");
        }
        var marker = new google.maps.Marker({
            position: myPt1,
            icon: myIcn1,
...
function clickedAddList() {
    var val = document.getElementById("inputList").innerHTML;
    val = val.replace(/\t/g, ' ');
    document.getElementById("inputList").innerHTML= val;
    addList(val);
}
